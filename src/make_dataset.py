...
def main(args):
    ...
    with open(config['vocab_path'], 'wb') as fout:
        pickle.dump(word_dict, fout)
    with open(config['word_set_path'], 'wb') as fout:
        pickle.dump(words, fout)
    logging.info('Word frequency and vocab saved in {}, {}'.format(config['word_set_path'], config['vocab_path']))
    ...
    long_test_pkl_path = os.path.join(args.dest_dir, 'long_test.pkl')
